...
closeModal() {
    ...
    handleDate(event) {
        ...
        // FIX timezone issue (convert to local midnight)
        this.date = selectedDate + 'T00:00:00';
    }
    ...
    this.appointments = this.appointments.map(item => {
        if (item.Id === this.editRecordId) {
            return {
                ...
                Appointment_Date_Time__c: this.date + 'Z',
...
openEditModal(row) {
    ...
    this.date = row.Appointment_Date_Time__c;
    this.status = row.Status__c;
}

saveAppointment() {
    const fixedDate = new Date(this.date + 'T' + this.time).toISOString();
